...
fn test_create_canister_from_fail() {
    ...
    let approved_amount = CREATE_CANISTER_CYCLES;
    env.icrc2_approve_or_trap(
        account1_1.owner,
        ApproveArgs {
            from_subaccount: account1_1.subaccount,
            spender: withdrawer1,
            amount: approved_amount.into(),
            expected_allowance: None,
            expires_at: None,
            fee: None,
            memo: None,
    ...
    expected_total_supply -= FEE;
    let expected_balance = env.icrc1_balance_of(account1_1);
    let expected_allowance = approved_amount;
    let blocks = env.icrc3_get_blocks(vec![(u64::MIN, u64::MAX)]).blocks;
    let error = env
        .create_canister_from(
            withdrawer1.owner,
            CreateCanisterFromArgs {
                ...
                spender_subaccount: withdrawer1.subaccount,
                created_at_time: None,
                amount: approved_amount.into(), // also costs FEE, therefore total_cost > approved_amount
                creation_args: None,
            },
        )
        .unwrap_err();
    assert_eq!(
        ...
        CreateCanisterFromError::InsufficientAllowance {
            allowance: approved_amount.into()
        }
    );
    assert_eq!(expected_balance, env.icrc1_balance_of(account1_1));
    assert_eq!(
